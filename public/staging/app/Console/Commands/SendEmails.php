...
class SendEmails extends Command
{
    ...
    public function handle()
    {
    	...
    	$query->where('is_send', 1)->where('mail_to', '<>', '');
    	$que_datas = $query->get();
		$message="";

		$monitor_mail= env('MONITORING_MAIL');
    	...
    	foreach($que_datas as $que_data) {
			if(!array_key_exists($que_data->send_que_id, $result)){
				...
				$mail_groups['is_claim'] = $que_data->is_claim;
				$mail_groups["to"] = [];
				$mail_groups["cc"] = [];
		...
		//BCC
		$bcclist = explode(',', $monitor_mail);
		$bcc=array();

		foreach($bcclist as $bcc_mail) {
			echo "BCCで送信[ email:".$bcc_mail."]";
...
//				Mail::to($mail_data->mail_to)->send(new AdoptionMail($data));
				if($mail_data['is_claim'] == "0"){
					Mail::to($mail_data['to'])->cc($mail_data['cc'])->bcc($bcc)->send(new AdoptionMail($data));
				}else if($mail_data['is_claim'] == "1"){
					Mail::to($mail_data['to'])->cc($mail_data['cc'])->bcc($bcc)->send(new ClaimMail($data));
				}
