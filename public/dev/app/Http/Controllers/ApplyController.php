...
class ApplyController extends BaseController
{
    ...
    public function index(Request $request) {
        ...
        //追加項目検索条件 プルダウン中身
        $search_list = !empty($optional_search) ? $this->service->getOptionalSearchListBox($user->primary_user_group_id) : null;

        //追加項目検索条件とプルダウン中身 本部
        if ($user->primary_user_group_id == $user->primary_honbu_user_group_id) {
            $hp_optional_search = $optional_search;
            $hp_search_list = $search_list;
        }else{
            $hp_optional_search = $this->service->getOptionalSearch($user->primary_honbu_user_group_id);
            $hp_search_list = !empty($hp_optional_search) ? $this->service->getOptionalSearchListBox($user->primary_honbu_user_group_id) : null;
        }
