...
class GroupTraderRelation extends Model
{
	...
	{
		$sql  = " select ";
    	$sql .= " f.trader_user_group_id ";
		$sql .= " , max(ug.name) trader_group_name ";
		$sql .= " , max(f.trader_role_key_code) trader_role_key_code ";
		$sql .= " from ";
		$sql .= "      f_show_records(". $user->id .") f";
        $sql .= " inner join user_groups ug ";
        $sql .= "     on f.trader_user_group_id = ug.id ";    	$sql .= " where  ";
    	$sql .= "     f.trader_user_group_id <>0 ";
		$sql .= " group by ";
		$sql .= " 	f.trader_user_group_id ";
    	$sql .= " order by  ";
		$sql .= "     f.trader_user_group_id ";
