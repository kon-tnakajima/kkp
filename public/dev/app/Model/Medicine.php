...
class Medicine extends BaseModel {
    ...
    public function listWithApplyIndex(Request $request, int $count, $user) {
        ...
        $sql .= ' ,jan_code asc ';
        $sql .= ' ,ug_sales.disp_order asc ';
        $sql .= ' ,ug_purchase.disp_order asc ';
        // $sql .= ' ,facility_id asc ';
        // $sql .= ' ,trader_name ';
        ...
        $total_count = isset($all_rec[0]) ? $all_rec[0]->total_count : 0;
    ...
    public function listWithApplySql(Request $request, int $count, $user) {
        ...
        $sql .= '    , count(1) over() as total_count ';
        ...
        $sql .= '    , ug_sales.name facility_name	';
        $sql .= '    , f_show_trader_name( 	';
        $sql .= '        CASE 	';
        $sql .= '            WHEN (base.pa_id is null and base.fp_id is null) 	';
        $sql .= '                THEN ug_fp_honbu_purchase.name	';
        $sql .= '            WHEN (base.pa_status = 100 and base.fp_id is null) 	';
        $sql .= '                THEN ug_fp_honbu_purchase.name	';
        $sql .= '            ELSE ug_purchase.name	';
        $sql .= '            END	';
        ...
        $sql .= '    , ug_sales.group_type	';
        $sql .= '    , base.pa_created_at	';
        $sql .= '    , base.pa_updated_at	';

        $sql .= "    , ta_forward.color_code forward_task_button_color_code	";
        $sql .= "    , '#999999' reject_task_button_color_code ";
        $sql .= "    , '#999999' backward_task_button_color_code ";
        $sql .= "    , vprpab.action_name_forward forward_task_button_text ";
        $sql .= "    , '見送り' reject_task_button_text ";
        $sql .= "    , case when vprpab.status_withdraw > 0 then '取下　' ";
        $sql .= "           when vprpab.status_remand > 0   then '差戻' end backward_task_button_text ";

        $sql .= '    , 0 forward_task_button_task_type ';
        $sql .= '    , 3 reject_task_button_task_type ';
        $sql .= '    , case when vprpab.status_withdraw > 0 then 1 ';
        $sql .= '           when vprpab.status_remand > 0   then 2 end backward_task_button_task_type ';

        $sql .= '    , vprpab.status_forward forward_task_button_status_forward	';
        $sql .= '    , vprpab.status_reject reject_task_button_status_reject	';
        $sql .= '    , case when vprpab.status_withdraw > 0 then vprpab.status_withdraw ';
        $sql .= '           when vprpab.status_remand > 0   then vprpab.status_remand end backward_task_button_status_backward ';



        $sql .= '    , case 	';
        $sql .= '    	when vprpab.status_forward between 20 and 40	';
        $sql .= '            then 1 	';
        $sql .= '        else - 1 	';
        $sql .= '        end as edit_pa_purchase_requested_price 	';
        $sql .= '    , case 	';
        $sql .= '        when f_calculate_status(base.fm_id,base.is_approval,base.fp_id,fp_honbu.id,base.pa_id,base.pa_status) < 50 and vprpab.status_forward = 50 	';
        $sql .= '            then 1 	';
        $sql .= '        else - 1 	';
        $sql .= '        end as edit_pa_purchase_estimated_price 	';
        $sql .= '    , case 	';
        $sql .= '        when f_calculate_status(base.fm_id,base.is_approval,base.fp_id,fp_honbu.id,base.pa_id,base.pa_status) < 60 and vprpab.status_forward >= 60 	';
        $sql .= '            then 1 	';
        $sql .= '        else - 1 	';
        $sql .= '        end as edit_pa_purchase_price 	';
        $sql .= '    , case 	';
        $sql .= '        when (f_calculate_status(base.fm_id,base.is_approval,base.fp_id,fp_honbu.id,base.pa_id,base.pa_status) < 80 and vprpab.status_forward >= 80) 	';
        $sql .= '        or     (f_calculate_status(base.fm_id,base.is_approval,base.fp_id,fp_honbu.id,base.pa_id,base.pa_status) < 60 and vprpab.status_forward >= 60)	';
        $sql .= '            then 1 	';
        $sql .= '        else - 1 	';
        $sql .= '        end as edit_pa_sales_price	';
        ...
        $sql .= '                    	';
        if ((isset($request->simple_search) && in_array($request->simple_search,['my_charge','my_charge_including_related','my_apply','is_applying','100','need_reques','need_estimate']))
            || isset($request->priority) || isset($request->pa_application_date_from) || isset($request->pa_application_date_to)
        ){//paやfpを抽出する検索条件であればinner joinでよい
            $sql .= '                    inner join ';
        } else{
            $sql .= '                    left join ';
        }
        ...
        // 私が担当 関連申請含むであればwhereをSQLの末尾ではなくここで設定する
        if ($request->simple_search == 'my_charge_including_related'){
            $sql .= ' where t1.pa_id in (select pa_id from f_price_adoptions_my_charge_and_source_paid('.$user->id.'))';
        }
        ...
        if(//未採用の行を抽出しない検索条件が設定されていれば、unionは不要
            isset($request->simple_search) && in_array($request->simple_search,['my_charge','my_charge_including_related','my_apply','is_applying','100'])
            || isset($request->user_group)
            || isset($request->status) && !in_array(1,$request->status)
            || isset($request->pa_application_date_from)
            || isset($request->pa_application_date_to)
            ...
            $sql .= '                and fm_p.sales_user_group_id = usr_p.primary_user_group_id 	';
        ...
        $sql .= '    inner join mv_privileges_row_price_adoptions_bystate as vprpab 	';
        $sql .= '        on vprpab.user_id = '.$user->id;
        $sql .= '        and vprpab.sales_user_group_id = coalesce(base.facility_id, usr.primary_user_group_id) 	';
        ...
        $sql .= '    inner join mv_privileges_row_price_adoptions as vpry 	';
        $sql .= '        on vpry.user_id = '.$user->id;
        $sql .= '        and vpry.sales_user_group_id = coalesce(base.facility_id, usr.primary_user_group_id) 	';
        ...
        $sql .= '    LEFT JOIN user_groups ug_sales ';
        $sql .= '        ON base.facility_id = ug_sales.id ';
        $sql .= '    LEFT JOIN user_groups ug_purchase ';
        $sql .= '        ON base.purchase_user_group_id = ug_purchase.id ';
        $sql .= '    LEFT JOIN user_groups ug_fp_honbu_purchase ';
        $sql .= '        ON fp_honbu.purchase_user_group_id = ug_fp_honbu_purchase.id ';
        ...
        if ($request->simple_search == 'need_request'){
            ...
            $sql .= "   and ug_sales.group_type = '文化連'";
        ...
        */
        ...
        // 販売中止品を含めない(=「販売中止品を含む」にチェックがない or 「私が担当」ではない)
        if (!(
                in_array($request->simple_search,['my_charge','my_charge_including_related','my_apply','is_applying'])//中止を含むにチェックがなくても私が担当では中止品を抽出する
                || isset($request->discontinuation_date)
            )) {
        ...
        // 申請日from
        if (!empty($request->pa_application_date_from)) {
            $sql .= " and cast(base.pa_application_date as date) >= '" . $request->pa_application_date_from ."'";
        }

        // 申請日to
        if (!empty($request->pa_application_date_to)) {
            $sql .= " and cast(base.pa_application_date as date) <= '" . $request->pa_application_date_to ."'";
        }
    ...
    // 承認申請一覧
    public function listWithApplyDetail(Request $request, $user) {
        $this->start_log();
        ...
        $sql  = '';
        $sql .= ' select ';
        $sql .= '      ug.name as facility_name ';
        $sql .= '     ,count(fm.id) over() as total_count ';
        $sql .= '     ,fm.adoption_date ';
        $sql .= '     ,fm.adoption_stop_date ';
        ...
        $sql .= ' from ';
        $sql .= '     facility_medicines fm';
        $sql .= '     inner join user_group_relations ugr';
        $sql .= '         on fm.sales_user_group_id = ugr.user_group_id';
        $sql .= '         and ugr.user_id = ' . $user->id;
        ...
        $sql .= '          on fm.sales_user_group_id=before3Detail.sales_user_group_id ';
        $sql .= '          and fm.medicine_id=before3Detail.medicine_id ';
        $sql .= '      left join (';
        $sql .= '          select ';
        $sql .= '              sales_user_group_id ';
        $sql .= '             ,medicine_id ';
        ...
        $sql .= '          on fm.sales_user_group_id=before2Detail.sales_user_group_id ';
        $sql .= '          and fm.medicine_id=before2Detail.medicine_id ';
        $sql .= '      left join (';
        $sql .= '          select ';
        $sql .= '              sales_user_group_id ';
        $sql .= '             ,medicine_id ';
        ...
        $sql .= '          on fm.sales_user_group_id=before1Detail.sales_user_group_id ';
        $sql .= '          and fm.medicine_id=before1Detail.medicine_id ';

        $sql .= ' where ';
        $sql .= '     fm.medicine_id = ' . $request->id;
        $sql .= '     and fm.is_approval = true';
        $sql .= '     and fm.deleted_at is null ';
        
        $sql .= ' order by ';
        $sql .= '       ug.disp_order, ug.name ';
...
*/
    ...
    public function getPrivilegesApply($user_id, $user_group_id, $status, $purchase_user_group_id) {
        ...
        $sql .= '      ,status_withdraw ';
        $sql .= ' from ';
        $sql .= '    mv_privileges_row_price_adoptions_bystate as a ';
        $sql .= '  where ';
    ...
    public function getApplyGroupList($user_id, $status) {
        ...
        $sql  = ' select';
        $sql .= ' 	f.sales_user_group_id as user_group_id';
        $sql .= ' 	, max(ug.name) as name ';
        $sql .= ' from mv_privileges_row_price_adoptions_bystate f';
        $sql .= ' inner join user_groups ug ';
        $sql .= ' 	on f.sales_user_group_id = ug.id ';
        $sql .= ' where f.user_id = '. $user_id;
        $sql .= ' 	and f.status = ' . $status;
        $sql .= ' 	and f.status_forward > 0';
        $sql .= ' group by';
        $sql .= '     f.sales_user_group_id';
        $sql .= ' order by';
        $sql .= '     max(ug.disp_order)';
    ...
    public function getEmailApply($status, $user_group_id, $exit_user_group_id, $purchase_user_group_id) {
        ...
        $sql .= ' from  ';
        $sql .= ' (select ';
        $sql .= '      f.status_forward ';
        $sql .= '      ,f.action_name_foward ';
        $sql .= '      ,f.status_reject';
        $sql .= '      ,f.action_name_reject';
        $sql .= '      ,f.status_remand';
        $sql .= '      ,f.status_withdraw';
        $sql .= '      ,u.email ';
        $sql .= '  from ';
        // $sql .= '    v_privileges_row_price_adoptions_bystate_20201002 as a ';
        $sql .= '    mv_privileges_row_price_adoptions_bystate as f ';
        $sql .= '     inner join users as u ';
        $sql .= '          on f.user_id = u.id';
        $sql .= '          and u.deleted_at is null ';
        $sql .= '  where f.sales_user_group_id = ' . $user_group_id;//f.user_id = ' . $user->id;
        $sql .= '      and f.purchase_user_group_id = ' . $purchase_user_group_id;
        $sql .= '      and f.status = ' . $status;
        $sql .= '      and f.status_forward > 0 ';
        $sql .= '      and u.is_adoption_mail = true';


        if (!empty($exit_user_group_id)) {
            $sql .= '      and u.primary_user_group_id <> ' . $exit_user_group_id;
        }
        ...
        if (env('ENV_TYPE') === 'staging') {
            $sql .= " and (t.email like '%bunkaren.or.jp' ";
            $sql .= "   or t.email like '%k-on.co.jp' ";
            $sql .= "   or t.email like '%example.com') ";
        }
    ...
    public function getOptionalSearch($user_group_id) {
        ...
        // $sql .= '     , CASE  ';
        // $sql .= '         WHEN optional_key1_is_search_disp = true ';
        // $sql .= '         or  optional_key2_is_search_disp = true  ';
        // $sql .= '         or  optional_key3_is_search_disp = true  ';
        // $sql .= '         or  optional_key4_is_search_disp = true  ';
        // $sql .= '         or  optional_key5_is_search_disp = true  ';
        // $sql .= '         or  optional_key6_is_search_disp = true  ';
        // $sql .= '         or  optional_key7_is_search_disp = true  ';
        // $sql .= '         or  optional_key8_is_search_disp = true  ';
        // $sql .= '             THEN true  ';
        // $sql .= '         ELSE false  ';
        // $sql .= '         END AS all_optional_key_is_search_disp ';
