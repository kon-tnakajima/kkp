...
@php($breadcrumbs = ['HOME'=>'./','所属申請一覧'=>'javascript:void(0);'])
...
@foreach($viewdata->get('list') as $account)
                        <tr>
                            ...
                            <td>
                                @if($account->status === \Config::get('const.account_status.application'))<a class="btn btn-primary btn-sm" href="{{ route('account.use.detail', $account->id) }}?page={{ $viewdata->get('pager')->current }}">開始</a>@endif
