...
@php($breadcrumbs = ['HOME'=>'/','採用管理'=>route('apply.index', ['page' => $detail->page, 'page_count' => $page_count]) ,'採用管理詳細'=>'javascript:void(0);'])
...
<div class="card card-primary apply_detail">
    ...
    <div class="d-flex justify-content-around">
        <!-- 申請可能な施設をselect -->
        <div>
            <span style="font-weight: bold;" class="mr5">施設</span>
        ...
        <div><span style="font-weight: bold;" class="mr5">ステータス</span><span class="status-label" style="display: inline-block;">{{ $detail->status_current_label }}</span></div>
    ...
    @endif
        ...
        @if ($sales_waku_kengen)
            ...
            @if ($purchase_waku_kengen)
                        <th></th><td></td>
            @else
                        @include('apply.components.detail_price_adoption', ['title' => '納入単価', 'type' => 'sales', 'decimal_point' => 0, 'detail' => $detail, 'errors' => $errors, 'require_flag' => $detail->edit_pa_sales_price > 0])
            @endif
        @endif
        ...
        @if ($sales_waku_kengen)
            ...
            @if ($purchase_waku_kengen)
                        <th colspan="2"></th>
            @else
                        @include('apply.components.detail_rate_pa', ['title' => '掛率', 'type' => 'sales', 'detail' => $detail, 'medicine' => $medicine])
            @endif
        @endif
                        <th>申請者</th>
        ...
        @if ($purchase_waku_kengen || $sales_waku_kengen)
            @if ($detail->edit_pa_purchase_user_group_id > 0)
                ...
                @foreach($detail->traders as $trader)
                                <option value="{{ $trader->id }}" @if ($detail->pa_purchase_user_group_id == $trader->id) selected @endif >{{ $trader->name }}</option>
                @endforeach
                            </select>
        ...
        @if ($sales_waku_kengen)
            ...
            @if ($purchase_waku_kengen)
                        <th colspan="2"></th>
            @else
                        @include('apply.components.detail_rate_pa', ['title' => '値引率', 'type' => 'sales', 'detail' => $detail, 'medicine' => $medicine])
            @endif
        @endif
                        <th>申請日時</th>
        ...
        @if ($purchase_waku_kengen)
                        <!-- 仕入 -->
                        @include('apply.components.detail_price_adoption', ['title' => '仕入単価', 'type' => 'purchase', 'decimal_point' => 0, 'detail' => $detail, 'errors' => $errors, 'require_flag' => $detail->edit_pa_purchase_price > 0])
        @endif
        @if ($sales_waku_kengen)
                        <!-- 納入 -->
            @if ($purchase_waku_kengen)
        ...
        @if ($purchase_waku_kengen)
                        <!-- 仕入掛率 -->
                        @include('apply.components.detail_rate_pa', ['title' => '掛率', 'type' => 'purchase', 'detail' => $detail, 'medicine' => $medicine])
        @endif
        @if ($sales_waku_kengen)
                        <!-- 納入掛率 -->
            @if ($purchase_waku_kengen)
        ...
        @if ($purchase_waku_kengen || ($sales_waku_kengen && $purchase_waku_kengen))
            ...
            @if ($purchase_waku_kengen)
                        <!-- 仕入値引率 -->
                        @include('apply.components.detail_rate_pa', ['title' => '値引率', 'type' => 'purchase', 'detail' => $detail, 'medicine' => $medicine])
            @endif
            @if ($sales_waku_kengen)
                        <!-- 納入値引率 -->
                @if ($purchase_waku_kengen)
                            ...
                            @if ($purchase_waku_kengen || $sales_waku_kengen)
                                @if ($detail->edit_facility_price_kengen > 0)
                                    ...
                                    <select class="form-control fp-edit" name="fp_purchase_user_group_id" id="fp_purchase_user_group_id" data-validate="empty">
                                        <option value="">選択してください</option>
                                            @foreach($detail->traders as $trader)
                                                <option value="{{ $trader->id }}" @if ($detail->fp_purchase_user_group_id == $trader->id) selected @endif >{{ $trader->name }}</option>
                                            @endforeach
                                    </select>
    ...
    @endif
            ...
            <table class="medicine-detail-table price-adoption-data fm-additional-info">
                <tbody>
                    ...
                    <tr>
                        <th @if($detail->edit_fm_adoption_stop_date > 0) @endif>採用中止日</th>
                        <td>
...
<script>
    ...
    /**
     * detail_price_adoption.blade.phpの中から呼び出される。
     * 単価を入力すると値引率と掛率が設定される
     */
    function priceRateDynamicUpdate(e) {
        const $inputElm = $(e.target);
        const inputElmId = $inputElm.attr('id');
         const value = $inputElm.val().trim();
        if (isNaN(value)) {
            // TODO 数値でない値が入力された場合の処理
            return
        }
        const packUnitPrice = Number($('#pa_basis_mediicine_price').text().replace("円", "").trim().replaceAll(",", ""));
        // 値引率の列にテキストを反映
        const nebikiristu = packUnitPrice === 0 || value === "" ? 0 : Math.round((packUnitPrice - value) / packUnitPrice * 10000) / 100;
        const kakeritsu = packUnitPrice === 0 || value === "" ? 0 : Math.round(value / packUnitPrice * 10000) / 100;
        $("#" + inputElmId + "_nebikiritsu").text(nebikiristu === 0 ? "" : nebikiristu > 0 ? `${nebikiristu} %` : `▲${Math.abs(nebikiristu)} %`);
        $("#" + inputElmId + "_kakeritsu").text(kakeritsu === 0 ? "" : kakeritsu > 0 ? `${kakeritsu} %` : `▲${Math.abs(kakeritsu)} %`);
