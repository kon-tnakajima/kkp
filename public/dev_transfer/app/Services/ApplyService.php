...
class ApplyService extends BaseService {
    ...
    public function getApplyDetailList(Request $request, $user) {
        ...
        $container_name = $request->flg === '1' ? 'priceAdoption' : 'medicine';
        $detail         = $this->{$container_name}->find($request->id);
        $m_id           = $request->flg === '1' ? $detail->m_id : $detail->id;
        ...
        return $this->medicine->listWithApplyDetail($request, $m_id, $user);
    }
...
/*
    ...
    public function regist(Request $request) {
        ...
        try {
            ...
            if (!empty($request->fm_id)) {
                ...
                $appryGroupPA = $this->priceAdoption->getData($request->apply_group, $medicine->id);
            } else {
                $sales_user_group_id = $user->primary_user_group_id;
            }


            if (empty($appryGroupPA)) {
                ...
                $medicinePriceTemp = $medicine->medicinePrice;
                $revision_date     = $medicine->medicine_price_revision_date;
                $basis_date        =  date('Y-m-d');

                if (!empty($medicinePriceTemp)) {
                    $basis_date = $medicinePriceTemp->start_date;
                } elseif (!empty($revision_date)) {
                    $basis_date = date('Y-m-d', strtotime($revision_date));
                } else {
                    $MinMedicinePrice = $this->medicinePrice
                        ->where('medicine_id', $request->id)
                        ->orderBy('start_date', 'asc')
                        ->first();
                    $basis_date = $MinMedicinePrice->start_date;
                }

                if (!empty($request->pa_basis_mediicine_price_date)) {
                    $data['basis_mediicine_price_date'] = $request->pa_basis_mediicine_price_date;
                    $basis_date                         = $request->pa_basis_mediicine_price_date;
                } else {
                    $data['basis_mediicine_price_date'] = $basis_date;
                }
                ...
                $medicine_price = (!empty($basisMedicinePrice)) ? $basisMedicinePrice->price * $medicine->packUnit->coefficient : 0;
                $data['basis_mediicine_price'] = (!empty($request->pa_basis_mediicine_price)) ? $request->pa_basis_mediicine_price : $medicine_price;
                ...
                $facility_medicine = $this->facilityMedicine->getData($sales_user_group_id, $medicine->id);

                if ($facility_medicine === null) {
                    $this->facilityMedicine->medicine_id         = $medicine->id;
                ...
                $facility_rec=$this->userGroup->where('id', $sales_user_group_id)->first();
                    $request->session()->flash(
                        'errorMessage',
                        '該当施設・薬品は、既に申請されています。[' . $facility_rec->name . ']'
                    );
                    \DB::rollBack();
            ...
            // }
            $mq->send_mail_add($mail_users, $priceAdoption);
    ...
    public function allow(Request $request) {
        ...
        try {
            ...
            $medicine_id = $request->m_id;

            // price_adoptions取得
            $medicine = $this->medicine->find($medicine_id);
            $user     = Auth::user();
            $data     = array();
            ...
            if (!empty($request->pa_id)) {
                $requestPA = $this->priceAdoption->find($request->pa_id);
            }
            
            $insert_status = [10,13,23,33,43,53,63,73,83,93,100,110,120,130];

            if (!empty($requestPA) && !in_array($requestPA->status, $insert_status)){
                $this->next($request);
            }
            elseif (empty($appryGroupPA)) {
                $data['status']           = $request->status_forward;
                $data['user_id']          = $user->id;
                $data['application_date'] = date('Y-m-d H:i:s');
                $data['purchase_user_group_id'] = 0;
                ...
                if (!empty($medicinePriceTemp)) {
                    $basis_date = $medicinePriceTemp->start_date;//現在有効な薬価
                } elseif (!empty($revision_date)) {
                    $basis_date = date('Y-m-d', strtotime($revision_date));//medicinesマスタの改定日
                } else {
                    $MinMedicinePrice = $this->medicinePrice
                        ->where('medicine_id', $medicine->id)
                        ->orderBy('start_date', 'asc')
                        ->first();
                    $basis_date = $MinMedicinePrice->start_date;//薬価改定日の最小値　　　　　　//TODO 現在有効な薬価がない、かつmedicinesマスタに改定日がない であれば、薬価基準日は申請日であるべき？
                }
                ...
                $facility_rec=$this->userGroup->find($sales_user_group_id);
                    $request->session()->flash(
                        'errorMessage',
                        '該当施設・薬品は、既に申請されています。[' . $facility_rec->name . ']'
                    );
            ...
            // }
            $mq->send_mail_add($mail_users, $pa);
    ...
    */
    ...
    public function calcBasisMediicinePrice($medicine,$inputDate) {
        ...
        $basis['basis_mediicine_price'] = (!empty($basisMedicinePrice)) ? $basisMedicinePrice->price * $medicine->packUnit->coefficient : 0;//(!empty($request->pa_basis_mediicine_price)) ? $request->pa_basis_mediicine_price : $medicine_price;
        return $basis;
    }
    ...
    public function registArrow(Request $request) {
        ...
        try {
            ...
            if (!empty($request->apply_group)){
                ...
                $primaryPA           = $this->priceAdoption->getData($user->primary_user_group_id, $request->id);
                // $sales_user_group_id = $user->primary_user_group_id;
            }
            ...
            if (!empty($requestPA)){logger('requestPA:'.$requestPA->id);}
            if (!empty($appryGroupPA)){logger('appryGroupPA:'.$appryGroupPA->id);}
            if (!empty($primaryPA)){logger('primaryPA:'.$primaryPA->id);}
            if (!empty($sales_user_group_id)){logger('sales_user_group_id:'.$sales_user_group_id);}
            if (!empty($facility_medicine)){logger('facility_medicine:'.$facility_medicine);}
...
/*
                ...
                if (!empty($medicinePriceTemp)) {
                    ...
                    $MinMedicinePrice = $this->medicinePrice
                        ...
                        ->orderBy('start_date', 'asc')
                        ->first();
                    $basis_date = $MinMedicinePrice->start_date;//TODO 現在有効な薬価がない、かつmedicinesマスタに改定日がない であれば、薬価基準日は申請日であるべき？
                }
...
*/
    ...
    public function confirm(Request $request) {
        ...
        if ($request->isCheck == '1') {
            if ($this->checkIndexApplyKomoku($request)) {
                $request->session()->flash(
                    'errorMessage',
        ...
        if ($request->isCheck == '2') {
            if ($this->checkDetailApplyKomoku($request)) {
        ...
        try {
            ...
            if (!empty($request->pa_purchase_apply_price)) {
                $priceAdoption->purchase_price = $this->removeComma($request->pa_purchase_apply_price); //仕入
            }

            if (!empty($request->pa_sales_apply_price)) {
                $priceAdoption->sales_price = $this->removeComma($request->pa_sales_apply_price); // 売上
            }
            ...
            if (!empty($request->pa_purchase_estimate_comment)) {
                $priceAdoption->purchase_estimate_comment = $request->pa_purchase_estimate_comment; //申請備考(業者－文化連)
            }

            if (!empty($request->pa_sales_estimate_comment)) {
                $priceAdoption->sales_estimate_comment = $request->pa_sales_estimate_comment; //申請備考(厚生連－文化連)
            }

            if (!empty($request->pa_comment)) {
                $priceAdoption->comment = $request->pa_comment; //申請備考(文化連)
            }

            if (!empty($request->pa_priority)) {
                $priceAdoption->priority = $request->pa_priority; //表示順
            }

            if (!empty($request->pa_basis_mediicine_price_date)) {
                $req_basis_mediicine_price_date = preg_replace('/(^\s+)|(\s+$)/u', '', $request->pa_basis_mediicine_price_date);

                if (!empty($req_basis_mediicine_price_date)) {
                    $priceAdoption->basis_mediicine_price_date = $request->pa_basis_mediicine_price_date;
                } 
            } 
    ...
    public function adopt(Request $request) {
        ...
        try {
            ...
            if ($fp_own === null) {
                ...
                $this->facilityPrice->purchase_price         = (!empty($request->pa_purchase_apply_price)) ? $this->removeComma($request->pa_purchase_apply_price) : $pa->purchase_price;//$pa->purchase_price;
                $this->facilityPrice->sales_price            = (!empty($request->pa_sales_apply_price)) ? $this->removeComma($request->pa_sales_apply_price) : $pa->sales_price;//$pa->sales_price;
                $this->facilityPrice->purchase_user_group_id = $pa->purchase_user_group_id;
                ...
                $fp_own->purchase_price = (!empty($request->pa_purchase_apply_price)) ? $this->removeComma($request->pa_purchase_apply_price) : $pa->purchase_price;//$pa->purchase_price;
                $fp_own->sales_price    = (!empty($request->pa_sales_apply_price)) ? $this->removeComma($request->pa_sales_apply_price) : $pa->sales_price;//$pa->sales_price;
                if (!empty($pa->end_date)){
        ...
        try {
            ...
            if ($fm_own === null) {
                ...
                for ($count = 1; $count <= 8; $count++) {
                    ...
                    if (!empty($request->{$key_name})) {
                        $this->facilityMedicine->{$key_name} = $request->{$key_name};
                    }
                }
                $this->facilityMedicine->save();
            } else {
            ...
            $this->facilityPrice->start_date             = (!empty($fp_parent->start_date)) ? $fp_parent->start_date : date('Y-m-d');
            $this->facilityPrice->end_date               = (!empty($fp_parent->end_date)) ? $fp_parent->end_date : date('2100-12-31');
    ...
    public function next(Request $request) {
        ...
        try {
            ...
            if ($pa->status <= 80 && ($request->status_forward == 90 || $request->status_forward == 100)) {
                $pa->approval_user_id = $user->id;
            }
            $pa->status      = $request->status_forward;
            ...
            if (!empty($request->pa_priority)) {
                $pa->priority = $request->pa_priority;
            }
    ...
    public function prev(Request $request) {
        ...
        try {
            ...
            /* 本部承認後は採用・見送りのみで取り下げ権限は付与しないが基本方針、取り下げが発生しても他のステータスのときと同様、ステータスを戻すのみとする
            //採用待ち
    ...
    public function edit(Request $request) {
        ...
        try {
            ...
            if (!empty($priceAdoption)) {
                // 仕入価
                if ($detail->edit_pa_purchase_apply_price == 1) {
                    $priceAdoption->purchase_price = $request->pa_purchase_apply_price !== null ? $this->removeComma($request->pa_purchase_apply_price) : null;
                }
                // 納入価
                if ($detail->edit_pa_sales_apply_price == 1) {
                    $priceAdoption->sales_price = $request->pa_sales_apply_price !== null ? $this->removeComma($request->pa_sales_apply_price) : null;
                }
                // 業者
                ...
                // //基準日薬価
                // if ($detail->edit_pa_besis_medicine_price == 1) {
                //     $priceAdoption->basis_mediicine_price = $request->pa_basis_mediicine_price !== null ? $request->pa_basis_mediicine_price : null;
                // }
                //薬価基準日
                // if ($detail->edit_pa_besis_medicine_price == 1) {
                //     $priceAdoption->basis_mediicine_price_date = $request->pa_basis_mediicine_price_date !== null ? $request->pa_basis_mediicine_price_date : null;
                // }


                if ($detail->edit_pa_besis_medicine_price == 1 && !empty($request->pa_basis_mediicine_price_date)) {
                    $req_basis_mediicine_price_date = preg_replace('/(^\s+)|(\s+$)/u', '', $request->pa_basis_mediicine_price_date);
                ...
                // else {
                //     $priceAdoption->basis_mediicine_price_date = null;
                // }
    ...
    public function reentry(Request $request, $user) {
        ...
        try {
            ...
            //$kengen_rec = $this->medicine->getPrivilegesApply($user->id,$user->primary_user_group_id,Task::STATUS_APPLYING);
            //$priceAdoption->next_status = $kengen_rec[0]->next_status;
            //$priceAdoption->reject_status = $kengen_rec[0]->reject_status;
            //$priceAdoption->withdraw_status = $kengen_rec[0]->withdraw_status;
    ...
    public function getConditions(Request $request) {
        ...
        if (!empty($request->is_search)) {
            ...
            $conditions['is_condition'] = ($request->is_condition == 'true' || $request->is_condition == '1')? true : false;
...
//         var_dump($conditions);
        ...
        if (!empty(session()->get('apply_conditions'))) {
            ...
            // もっと検索条件を見る
            if (!empty($conditions['is_condition'])) {
                $request->is_condition = $conditions['is_condition'];
            }
    ...
    // 採用申請一覧項目nullチェック
    public function checkIndexApplyKomoku($request) {
        $this->start_log();
    ...
    // 採用申請詳細項目nullチェック
    public function checkDetailApplyKomoku($request) {
        $this->start_log();
