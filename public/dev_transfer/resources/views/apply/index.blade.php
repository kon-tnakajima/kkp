...
.apply table tr td.adoption-stop-apply{background-color:#E6E6E6}
 ...
 * 「もっと検索条件を見る」
 */
$is_condition = $viewdata->get('conditions')['is_condition'];
?>
                <a
                    class="anchor anchor-accordion anchor-search-condition {{ $is_condition ? 'open' : '' }}"
                    href="#collapseExample" data-toggle="collapse"
                    aria-expanded="{{ $is_condition }}"
                    aria-controls="collapseExample"
                    onClick="const val = $('#collapseExample').hasClass('show'); $('#is_condition').val(!val);"
                >もっと検索条件を見る</a>
                <input type="hidden" id="is_condition" name="is_condition" data-search-item="is_condition" value="{{ $is_condition }}">
            </div>
            <div class="row">
<?php
/*
...
?>
                <div class="col-10 d-flex">
                    <div class="d-flex mr30 align-items-center block w100p">
                        <input
                            class="form-control"
                            type="search"
    ...
    ?>
                <div class="col-2">
                    <button class="w100p btn btn-primary btn-table-search" type="button">
     ...
     */
    ?>
               <table class="ml15 search-collapse">
                   <tr>
...
?>
            <div
                class="collapse @if ($is_condition) show @endif"
                id="collapseExample"
            >
...
?>
    ...
    @foreach ($viewdata->get('list') as $apply)
                        <tr>
                            <!-- No -->
                            <td class="no text-center @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ( $apply->flg_in_charge_of_adoption ) target-apply @endif @if ($viewdata->get('list')->total() < 100) col-w-2 @elseif ($viewdata->get('list')->total() < 1000) col-w-3 @elseif ($viewdata->get('list')->total() < 10000) col-w-4 @elseif ($viewdata->get('list')->total() < 100000) col-w-5 @endif">
                                {{ $loop->iteration + ( $viewdata->get('page_count') * ($viewdata->get('pager')->current -1) ) }}
                            </td>
                            <!-- GS1販売/販売メーカー -->
                            <td class="sales-info @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
                                <div>
        ...
        @endif
                            ...
                            <!-- 商品名/規格容量 -->
                            <td
                                class="medicine-info medicine @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ( $apply->flg_in_charge_of_adoption ) target-apply @endif"
                            ...
                            <!-- 包装薬価係数 -->
                            <td class="coefficient text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
                                {{ $apply->coefficient }}
                            </td>
                            <!-- 単位薬価 -->
                            <td class="medicine-price text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
                                @if (isset($apply->medicine_price))
                                    {{ number_format(round($apply->medicine_price, 2),2) }}
                                @endif
                            </td>
                            <!-- 包装薬価 -->
                            <td class="pack-unit-price text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
        ...
        @if ($purchase_waku_kengen)
                            <!-- 要望仕入価格 -->
                            <td class="purchase-requested-price text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
        ...
        @if ($purchase_waku_kengen)
                            <!-- 要望仕入値引率 -->
                            <td class="purchase-requested-nebiki-rate text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
            @if (isset($apply->purchase_requested_price))
        ...
        @if ($purchase_waku_kengen)
                            <!-- 見積仕入価格 -->
                            <td class="purchase-estimated-price text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
        ...
        @if ($purchase_waku_kengen)
                            <!-- 見積仕入値引率 -->
                            <td class="purchase-estimated-nebiki-rate text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
            @if (isset($apply->purchase_estimated_price))
        ...
        @if ($purchase_waku_kengen)
                            <!-- 仕入価格 -->
                            <td class="purchase-price text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
        ...
        @if ($purchase_waku_kengen)
                            <!-- 仕入値引率 -->
                            <td class="purchase-nebiki-rate text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
            @if (isset($apply->purchase_price))
        ...
        @if ($sales_waku_kengen)
                            <!-- 納入価格 -->
                            <td class="sales-price text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
        ...
        @if ($sales_waku_kengen)
                            <!-- 値引率(納入) -->
                            <td class="sales-rate text-right @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
            @if (!is_null($apply->getSalesPrice()))
        ...
        @endif
                            <!-- 施設/業者 -->
                            <!-- <td class="trader-facillity @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
                                <div>
                            ...
                            <!-- 施設 -->
                            <td class="trader-facillity text-left @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
                                {{ $apply->facility_name }}
                            </td>
                            <!-- 業者 -->
                            <td class="trader-facillity text-left @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
                            ...
                            <!-- ステータス/申請日 -->
                            <td class="status-date @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
                                <div>
                                    <span
                                        class="badge flex-center-self"
        ...
        @endif
                            ...
                            <!-- タスク -->
                            <td class="task text-center @if ( $apply->adoption_stop_date_fg ) adoption-stop-apply @endif @if ($apply->flg_in_charge_of_adoption ) target-apply @endif">
                                <div>
        @if (!empty($apply->button['url']) and $apply->status != Task::STATUS_DONE)
                                    ...
                                    >
                                    <button
                                         style="background-color: {{ $apply->button['style'] }}; color: white;"
                                         class="btn2 btn2-sm"
                                         type="submit"
...
/*
const hideCols = function ($elm) {
    const target = $elm.data("target");
    const hide = $elm.data("hide").split(",");
    $(target + " th, " + target + " td").each(function(ind, elm) {
        $(elm).show();
    });
    for (colClass of hide) {
        $(target + " ." + colClass.trim()).each(function(ind, elm) {
            $(elm).hide();
        });
    }
}
jQuery(function() {
    hideCols($('input[name="mode_radios"]:checked'));
    $('input[name="mode_radios"]').change(function() {
        hideCols($(this));
    });
});
*/
