...
class AccountController extends BaseController
{
     ...
     * @return json
     */
    public function remakeToken(Request $request)
    {
        $userId = Auth::user()->id;
        $token = $this->service->updateUserToken($userId);
     ...
     * @return json
     */
    public function updatePrimaryUserGroupId(Request $request)
    {
        $primaryUserGroupId = $request->id;
        $this->service->updatePrimaryUserGroupId($primaryUserGroupId);
        return response()->json([
            'ok' => true
        ], 200);
    }
