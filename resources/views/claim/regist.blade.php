                ...
                <thead>
                    ...
                    @component('claim.components.regist_item', ['label' => '施設', 'property' => 'facility', 'require' => true, 'errors' => $errors])
                        ...
                        @foreach($viewdata->get('user_groups') as $user_group)
                            <option value="{{$user_group->id}}" @if( $user_group->id == $viewdata->get('select_facility') ) selected @endif >{{$user_group->name}}</option>
                        @endforeach 
                        </select>
                    @endcomponent
                    ...
                    @component('claim.components.regist_item', ['label' => '施設', 'property' => 'facility', 'require' => false, 'errors' => $errors])
                        <p class="form-control-static">{{ $viewdata->get('user_groups')[0]->name }} </p>
                        @include('layouts.input_hidden', ['parent' => '[claim.regist]', 'name' => 'facility','id' => 'facility', 'value' => $viewdata->get('user_groups')[0]->id])
                    @endcomponent
                    @endif
