...
@php($breadcrumbs = ['HOME'=>'./','ロール一覧'=>'javascript:void(0);'])
...
<div class="card card-primary">
    ...
    <div class="row mt30">
        <div class="tar">
            <a class="ml30 btn btn-default" href="{{ route('role.add') }}?page={{ $viewdata->get('pager')->current }}">新規登録</a>
        ...
        <div class="row mt10">
            ...
            <div class="col-4 d-flex">
                <div class="d-flex mr10 align-items-center block">
                    <p class="w200 mr10 fwb text">種別</p>
                    <div class="d-flex">
                        <div class="d-flex w200p align-items-center block">
                            <select class="w100 form-control" data-search-item="group_type">
                                <option value="" @if($viewdata->get('conditions')['group_type'] == "") selected @endif>全て</option>
@foreach($viewdata->get('types') as $type)
                                <option value="{{$type->name}}" @if( $type->name == $viewdata->get('conditions')['group_type'] ) selected @endif>{{$type->name}}</option>
@endforeach
                            </select>
                        </div>
                    </div>
        ...
        <div class="row mt10">
            <div class="col-12">
                <table class="table table-primary table-small" id="role_lists">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>名前</th>
                    ...
                    <tbody>
@foreach($viewdata->get('list') as $role)
                        <tr>
                            <td>{{ $role->id }}</td>
                            <td>{{ $role->name }}</td>
                            <td>{{ $role->key_code }}</td>
                            ...
                            <td>
                                <a class="btn btn-primary btn-sm" href="{{ route('role.detail', ['id' => $role->id]) }}?page={{ $viewdata->get('pager')->current }}">編集</a>
                                <a class="btn btn-primary btn-sm" href="{{ route('role.copy', ['id' => $role->id]) }}?page={{ $viewdata->get('pager')->current }}">複製</a>
                            </td>
                        </tr>
@endforeach
                    </tbody>
