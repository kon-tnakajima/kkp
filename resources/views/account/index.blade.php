...
<div class="card card-primary">
    ...
    <div class="row mt30">
        <div class="col-2 tar">
            <!--div class="block"><a class="ml30 btn btn-default" href="{{ route('usergroup.add') }}">新規登録</a></div-->
...
@foreach($viewdata->get('list') as $account)
                        <tr>
                            ...
                            <td>{{ $account->name }}</td>
                            <td>{{ $account->email }}</td>
                            <td>{{ $account->user_group_id }}</td>
                            <td>{{ $account->user_group_name }}</td>
                            <td>{{ $account->updated_at }}</td>
                            <td>
                                @if($account->status == App\Services\AccountService::APPLICATION_STATUS )<a class="btn btn-primary btn-sm" href="{{ route('account.detail', $account->id) }}">許可</a>@endif
                                @if($account->status == App\Services\AccountService::PERMISSION_STATUS )<a class="btn btn-primary btn-sm" href="{{ route('account.start', $account->id) }}">開始</a>@endif
                                @if($account->status == App\Services\AccountService::START_STATUS ) 開始 @endif
                                @if($account->status == App\Services\AccountService::REJECTION_STATUS ) 却下 @endif
                            </td>
                        </tr>
@endforeach
                    </tbody>
